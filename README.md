...
built from: lein new re-frame reframe-hero-app +cider +test +garden +re-com +routes
...
* [`src/reframe_hero_app/styles.cljs`](src/reframe_hero_app/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/reframe_hero_app/`](src/reframe_hero_app/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/reframe_hero_app/core.cljs): contains the SPA entry point, `init`
* [`test/reframe_hero_app/`](test/reframe_hero_app/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
5. Clone this repo and open a terminal in the `reframe-hero-app` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/reframe_hero_app/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns reframe-hero-app.example
  (:require [reframe-hero-app.config :as config])
